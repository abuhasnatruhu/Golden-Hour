...
export function parseEnhancedURL(pathname: string, searchParams?: URLSearchParams): {
  locationName?: string
  date?: string
  lat?: number
  lng?: number
  ...
  try {
    ...
    // Check if we have coordinates in search params
    if (searchParams) {
      const searchLat = searchParams.get("lat")
      const searchLng = searchParams.get("lng") || searchParams.get("lon")
    ...
    if (params.length === 1) {
      ...
      if (coordPattern.test(params[0])) {
        ...
        const genericKeywords = ["location", "place", "city", "coordinates", "coord", "coords"]
        if (!genericKeywords.includes(params[0].toLowerCase())) {
          locationName = params[0].replace(/-/g, " ")
        }
      }
    } else if (params.length === 2) {
      // Two params - location/coords + date
      // Check if first param is a generic keyword like "location"
      const genericKeywords = ["location", "place", "city", "coordinates", "coord", "coords"]
      ...
      if (coordPattern.test(params[0])) {
        ...
        lat = parseFloat(latStr)
        lng = parseFloat(lngStr)
      } else if (!genericKeywords.includes(params[0].toLowerCase())) {
        // Only use as location name if it's not a generic keyword
        locationName = params[0].replace(/-/g, " ")
      }
